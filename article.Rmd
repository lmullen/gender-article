...
ahr_predicted %>%
  ...
  filter(gender_author == "female",
         !our_category %in% c(NA, 
                              "Latin and South America, Carribean",
                              "Middle East")) %>%
  select(year, gender_reviewer, our_category) %>% 
  group_by(year, gender_reviewer, our_category) %>%  
  summarize(n = n()) %>%
  spread(gender_reviewer, n, fill = 0) %>% 
  mutate(female_proportion = female / (female + male + unknown),
         male_proportion = male / (female + male + unknown),
...
ggplot(aes(x = year, y = proportion, fill = gender)) + 
  ...
  xlim(1970, 2012) +
  ggtitle("Gender of reviewers of female AHR authors by category, 1970-2012") +
  xlab(NULL) + ylab(NULL) +
  theme(legend.position = "bottom") +
  geom_hline(yintercept = 0.5) +
  scale_fill_brewer(palette = article_palette,
