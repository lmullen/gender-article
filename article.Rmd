...
Nevertheless, the methodology that we have described here does take advantage of the datasets available to historians. While we agree with the category of critique that we have cited, we do not think that they should deter scholars from making use of our method. While such critiques point out genuine problems, they also seldom contain concrete proposals that can actually be implemented in historical research.^[Discussing exactly this problem of gender/sex binaries in large corpora, Penelope Eckert argues that "binary sex-class gets us pretty far" and argues that "at the moment" it is not "whether there's any way we can code large corpora for something subtler about gender than sex 
class" [Eckert 2012].] The predictions that our method provides are by no means intended to be taken as the entirety of what historians can say about gender in the past. But they do provide the ability to uncover data about gender where it was previously unknown, thus providing further grist to the mill of more traditional methods. Furthermore, the gender package is entirely open-source, and we have provided detailed documentation for its code and underlying data so that researchers who wish to build upon it can modify the package accordingly. By building as much flexibility and transparency into the method as possible, we hope that others can continue to make just these kinds of improvements.

As it currently stands, the gender package meets a methodological need. As of August 2015, the gender package has been downloaded more than 21,000 times and incorporated as a pedagogical tool within digital humanities courses [Jockers 2014b].^[The download counts come from the [download logs from RStudio's CRAN mirror](http://cran-logs.rstudio.com/) as tabulated by Gabor Csardi's [cranlogs package](http://cranlogs.r-pkg.org/). [Csardi 2015] RStudio is the only CRAN mirror to release its logs.)] Researchers have used it to uncover instances gender inequity in a range of different areas, from biases in students' teaching evaluations to disparities between attendees and authors at the annual Digital Humanities Conference [Schmidt 2015, Weingart 2015]. These implementations help illustrate the value of the method. Its major intervention is to algorithmically inferring gender in textual sources in a way that takes into account change over time. Rather than trying to shoehorn existing computer science methods to fit humanities research, the gender package reverses this traditional disciplinary relationship. It uses a humanistic starting point---the historical relationship between gender and naming practices---to contribute a new method to computer science.
