...
**Do you have the mapping you use from the AHR's 68 categories to the 8 or so you used? With that, I just have one or two more lines to facet this plot.**
...
ahr_predicted %>%
  ...
  select(female_proportion, male_proportion, unknown_proportion,
         year, category) %>% 
  gather(gender, proportion, -year, -category) -> temp
ggplot(aes(x = year, y = proportion, fill = gender)) + 
  geom_bar(binwidth = 1, stat = "identity") +
  ggtitle("Gender of reviewers of female book authors in AHR") +
  xlab(NULL) + ylab(NULL) +
