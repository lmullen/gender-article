...
Our program allows us to quantitatively measure the scope of the problem on a large scale and how it has changed over time. [to write later: include thoughts how this is even more important as gender discrimination turns away from the out-in-the-open kind of discrimination to more subtle disadvantages and systemic problems.]
 
### Conclusion
 
Our algorithm for predicting gender from first names, when combined with historical datasets in the Gender package, provides a powerful new method for researchers interested in the study of gender. The digital turn has made available very large datasets, yet in the midst of abundance many humanists continue to deal with the problem of scarcity in their sources. Our method uses the abundant existing datasets to infer more information from scarce datasets. The temporal dimension of our work matches historical questions to a historical method, providing much improved results over simple, anachronistic lookup methods. This method is extensible to any time or place for which there is a suitable dataset of names from which to make predictions about gender. This method is potentially useful to anyone who has a dataset of names, especially if those names can be associated with particular dates. As we have demonstrated in our study of gatekeeping in the American historical profession, our method can reveal important insights about gender even from datasets which do not otherwise include that information, and it can do so over long historical periods. 
