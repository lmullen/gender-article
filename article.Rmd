...
ahr_predicted %>%
  ...
  select(female_proportion, male_proportion, unknown_proportion,
         year, our_category) %>% 
  gather(gender, proportion, -year, -our_category) %>%
ggplot(aes(x = year, y = proportion, fill = gender)) + 
  geom_bar(binwidth = 1, stat = "identity") +
  facet_wrap(~ our_category) +
  xlim(1970, 2012) +
  ggtitle("Gender of AHR authors by book category, 1970-2012") +
  xlab(NULL) + ylab(NULL) +
  theme(legend.position = "bottom") +
  geom_hline(yintercept = 0.5) +
  scale_fill_brewer(palette = article_palette,
