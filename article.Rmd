...
The simplest way to use the package would be to call the `gender()` function, specifying the method, in this case the Social Security Administration dataset since all the names are after 1930, and specifying a single range of years to make the predictions. However, we can instead use the `gender_df()` function provided by the package. By passing it our data frame of people and specifying which columns contain the names and birth years (or a columns containing a minimum and maximum for a range of years), we can efficiently predict the gender for each person.^[See the [package vignette](https://cran.rstudio.com/web/packages/gender/vignettes/predicting-gender.html) for more details about how to use the package effectively, including suggestions about performance: `browseVignettes( "gender")`.]

```{r}
gender_df(editors, name_col = "name", year_col = "birth_year")
```
